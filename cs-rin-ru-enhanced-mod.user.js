...
if (options.infinite_scrolling && $("[title='Click to jump to page…']").length > 0) {
    ...
    if (styleElement) {
        // Increase size of the nav bar - append to style so it doesn't interfere with other forum styling
        styleElement.textContent += `\n[name="page_nav"] {font-size: ${navBarSize};}`;
    }
            ...
            $(".gensmall div+ div:not(:has([title='Reply with mentioning']))").each(function () {
                ...
                if (!quickReplyPanel) {
                    ...
                    $(this).find('[title="Reply with mentioning"]').on("click", function () {
                        let postBody = `@[b][url=${FORUM_BASE_URL}memberlist.php?mode=viewprofile&u=${authorID}]${decodeURI(author)}[/url][/b] `;
                        if (options.mentioning === 2) { //Author and post
...
function setupPostPreview() {
    ...
    $("a.postlink").each((_, e) => {
        ...
        if (link.includes("t=") && options.topic_preview) {
            const link = post.href.split("&view=unread")[0].split("&p=")[0];
            const getIndex = (posts, link) => options.topic_preview_option === 2 ? posts.length - 2 : 1;
            return previewElement(post, link, getIndex);
        }
